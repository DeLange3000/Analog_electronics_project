function mosFinPrintSizesAndOpInfo(fid, circuit)
% MOSFINPRINTSIZESANDOPINFO print sizes and operating point information of MOS
% transistors in a circuit and its subcircuits.
%
%   mosFinPrintSizesAndOpInfo(FID, CIRCUIT) prints the sizes and
%   operating point information of the transistors in the given
%   CIRCUIT and its subcircuits to the given file identifier FID. Two
%   file identifiers are automatically available and need not be
...
if (circuit.nnmos > 0) | (circuit.npmos > 0) 
  ...
  for i = 1:circuit.nnmos
    if not(isfield(circuit.(circuit.nmosList{i}), 'newWidth'))
      circuit.(circuit.nmosList{i}).newWidth = ...
	  circuit.(circuit.nmosList{i}).w;
    end
    fprintf(fid, '%s: ', circuit.(circuit.nmosList{i}).name)
    fprintf(fid, ...
	'W=%sm\tWeff=%sm\tL=%sm\tnFingers=%d\tfingerWidth=%s\tnFins=%d\n', ...
	eng(circuit.(circuit.nmosList{i}).w, [], 2), ...
	eng(circuit.(circuit.nmosList{i}).newWidth, [], 2), ...
	eng(circuit.(circuit.nmosList{i}).lg), ...
	circuit.(circuit.nmosList{i}).nFingers, ...
	eng(circuit.(circuit.nmosList{i}).newWidth / ...
	circuit.(circuit.nmosList{i}).nFingers, [], 2), ...
	circuit.(circuit.nmosList{i}).nFins); 
  end
  for i = 1:circuit.npmos
    if not(isfield(circuit.(circuit.pmosList{i}), 'newWidth'))
      circuit.(circuit.pmosList{i}).newWidth = ...
	  circuit.(circuit.pmosList{i}).w;
    end
    fprintf(fid, ...
	'%s: W=%sm\tWeff=%sm\tL=%sm\tnFingers=%d\tfingerWidth=%s\tnFins=%d\n', ...
	circuit.(circuit.pmosList{i}).name, ...
	eng(circuit.(circuit.pmosList{i}).w, [], 2), ...
	eng(circuit.(circuit.pmosList{i}).newWidth, [], 2), ...
	eng(circuit.(circuit.pmosList{i}).lg), ...
	circuit.(circuit.pmosList{i}).nFingers, ...
	eng(circuit.(circuit.pmosList{i}).newWidth / ...
	circuit.(circuit.pmosList{i}).nFingers, [], 2), ...
	circuit.(circuit.pmosList{i}).nFins); 
  end
